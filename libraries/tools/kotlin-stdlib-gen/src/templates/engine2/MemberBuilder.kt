...
private fun getDefaultSourceFile(f: Family): SourceFile = when (f) {
    Family.Iterables, Family.Collections, Family.Lists -> SourceFile.Collections
    Family.Sequences -> SourceFile.Sequences
    Family.Sets -> SourceFile.Sets
    Family.Ranges, Family.RangesOfPrimitives, Family.ProgressionsOfPrimitives -> SourceFile.Ranges
    Family.ArraysOfObjects, Family.InvariantArraysOfObjects, Family.ArraysOfPrimitives -> SourceFile.Arrays
    Family.Maps -> SourceFile.Maps
    Family.Strings -> SourceFile.Strings
    Family.CharSequences -> SourceFile.Strings
    Family.Primitives, Family.Generic -> SourceFile.Misc
}
...
class MemberBuilder(
    ...
    fun build(builder: Appendable) {
        ...
        fun renderType(expression: String, receiver: String, self: String): String {
            ...
            while (t.hasMoreTokens()) {
                ...
                answer.append(when (token) {
                    ...
                    "TCollection" -> {
                        when (family) {
                            Family.CharSequences, Family.Strings -> "Appendable"
                    ...
                    "T" -> {
                        when (family) {
                            Family.Generic -> "T"
                            Family.CharSequences, Family.Strings -> "Char"
                            Family.Maps -> "Map.Entry<K, V>"
                            else -> primitive?.name ?: token
                        }
                    }
                    "TRange" -> {
                        when (family) {
                            Family.Generic -> "Range<T>"
                            else -> primitive!!.name + "Range"
                        }
                    }
                    "TProgression" -> {
                        when (family) {
                            Family.Generic -> "Progression<out T>"
        ...
        val self = (when (family) {
            Family.Iterables -> "Iterable<$isAsteriskOrT>"
            Family.Collections -> "Collection<$isAsteriskOrT>"
            Family.Lists -> "List<$isAsteriskOrT>"
            Family.Maps -> "Map<out K, V>"
            Family.Sets -> "Set<$isAsteriskOrT>"
            Family.Sequences -> "Sequence<$isAsteriskOrT>"
            Family.InvariantArraysOfObjects -> "Array<T>"
            Family.ArraysOfObjects -> "Array<${isAsteriskOrT.replace("T", "out T")}>"
            Family.Strings -> "String"
            Family.CharSequences -> "CharSequence"
            Family.Ranges -> "ClosedRange<$isAsteriskOrT>"
            Family.ArraysOfPrimitives -> primitive?.let { it.name + "Array" } ?: throw IllegalArgumentException("Primitive array should specify primitive type")
            Family.RangesOfPrimitives -> primitive?.let { it.name + "Range" } ?: throw IllegalArgumentException("Primitive range should specify primitive type")
            Family.ProgressionsOfPrimitives -> primitive?.let { it.name + "Progression" } ?: throw IllegalArgumentException("Primitive progression should specify primitive type")
            Family.Primitives -> primitive?.let { it.name } ?: throw IllegalArgumentException("Primitive should specify primitive type")
            Family.Generic -> "T"
        })
        ...
        fun effectiveTypeParams(): List<GenericFunction.TypeParameter> {
            ...
            if (family == Family.Generic) {
                if (parameters.none { it.name == "T" })
                    parameters.add(GenericFunction.TypeParameter("T"))
                return parameters
            } else if (primitive == null && family != Family.Strings && family != Family.CharSequences) {
                ...
                for (implicit in implicitTypeParameters.reversed()) {
                    if (implicit.name != "*" && parameters.none { it.name == implicit.name }) {
                        parameters.add(0, GenericFunction.TypeParameter(implicit.name))
                    }
                }
        ...
        doc?.let { methodDoc ->
            ...
            if (family == Family.Sequences && sequenceClassification.isNotEmpty()) {
                builder.append(" *\n")
